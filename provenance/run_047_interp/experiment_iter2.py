...
def train_model_fast(model, train_loader, val_loader, device, max_epochs=10):
    ...
    for epoch in range(max_epochs):
        ...
        if epoch % 3 == 0:
            ...
            print(f'Epoch {epoch}: Train Loss: {train_loss/(batch_idx+1):.4f}, '
...
def measure_feature_reuse_fast(model1, model2, data_loader, device):
    ...
    # Simple transport: use Hungarian algorithm for matching
    # This is faster than full optimal transport
    n1, n2 = cost_matrix.shape
    
    if n1 <= n2:
        # Pad cost matrix to make it square
        padded_cost = np.pad(cost_matrix, ((0, n2-n1), (0, 0)), constant_values=1e6)
        row_ind, col_ind = ot.emd(np.ones(n2)/n2, np.ones(n2)/n2, padded_cost)
        # Keep only valid matches
        valid_matches = [(i, col_ind[i]) for i in range(n1) if padded_cost[i, col_ind[i]] < 1e5]
    else:
        padded_cost = np.pad(cost_matrix, ((0, 0), (0, n1-n2)), constant_values=1e6)
        row_ind, col_ind = ot.emd(np.ones(n1)/n1, np.ones(n1)/n1, padded_cost)
        valid_matches = [(row_ind[j], j) for j in range(n2) if padded_cost[row_ind[j], j] < 1e5]
...
def run_experiment(seed):
    ...
    if len(reused_pairs) > 0:
        ...
        if len(model1_acts) > 0:
            ...
            consistency_improvement = abs(corr_reused) - abs(random_corr)
...
def main():
    ...
    final_results = {
        ...
        'std': {
            'reuse_score': np.std(reuse_scores) if len(reuse_scores) > 1 else 0,
            'consistency_improvement': np.std(consistency_improvements) if len(consistency_improvements) > 1 else 0
        },
        'p_values': {
