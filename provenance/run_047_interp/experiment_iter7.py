...
def main():
    ...
    # Signal detection - ensure boolean conversion
    signal_detected = bool(np.mean(reuse_scores) > random_baseline * 1.5 and p_val_random < 0.05)
    ...
    # Calculate ablation mean safely
    ablation_values = []
    for r in all_results[:3]:
        if 'ablations' in r and 'bins_20' in r['ablations']:
            ablation_values.append(r['ablations']['bins_20'])
    
    ablation_mean = float(np.mean(ablation_values)) if ablation_values else 0.0
    ...
    final_results = {
        ...
        'ablation_results': {
            'bins_20': ablation_mean
        },
        'convergence_status': convergence_status,
        'signal_detected': signal_detected,  # Now guaranteed to be Python bool
