...
def compute_comprehensive_baselines(model_class1, model_class2, train_loader, val_loader, 
    ...
    for i in range(3):
        ...
        # Quick training
        m1, _, _ = train_model(m1, train_loader, val_loader, device, max_epochs=3)
        
        torch.manual_seed(3100 + i)
        m2, _, _ = train_model(m2, train_loader, val_loader, device, max_epochs=3)
...
def run_experiment():
    ...
    for arch1_name, arch2_name, arch1_class, arch2_class in arch_pairs:
        ...
        for seed in range(num_seeds):
            ...
            feat_loader = DataLoader(feat_data, batch_size=64, shuffle=False)
    ...
    train_loader_baseline = DataLoader(train_data, batch_size=64, shuffle=True, num_workers=2)
    val_loader_baseline = DataLoader(val_data, batch_size=64, shuffle=False, num_workers=2)
    feat_loader_baseline = DataLoader(feat_data, batch_size=64, shuffle=False)
    ...
    model1, _, _ = train_model(model1, train_loader_baseline, val_loader_baseline, device, max_epochs=3)
    model2, _, _ = train_model(model2, train_loader_baseline, val_loader_baseline, device, max_epochs=3)
